class alu_timing_seq extends uvm_sequence #(alu_seq_item);
  `uvm_object_utils(alu_timing_seq)

  function new(string name = "alu_timing_seq");
    super.new(name);
  endfunction
  ...
  task send_idle_cycles(int count);
    repeat(count) begin
      ...
      if (!req.randomize() with {
        ...
        MODE      == 1'b1;    // Hardcoded Arithmetic Mode
        CIN       == 1'b0;
  ...
  task body();
    ...
    if (!req.randomize() with {
      ...
      CE        == 1'b1;
      RST       == 1'b0;
      CMD       == 4'b0000; MODE == 1'b1; CIN == 1'b0;
